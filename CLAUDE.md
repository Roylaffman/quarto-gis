...
- Going back over each create .qmd file and finding ways to improve it removing em dashes from all text,. Focus on more Human sounding written sections with shorter sentences and careful paragraph constructuren.
- Create an agent whos job is to read the .qmd file, and find ways to improve each report, take out AI elements in written text , and fact check information
- Create an agent who uses skilss and tools needed to start adding the upgraded .qmd files to the substack workflow, where the article is added to the substack template in the Obsidian vault. It will